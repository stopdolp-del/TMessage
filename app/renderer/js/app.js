 ...
 */
import { api, setToken, setRefreshToken, getToken } from './api.js';
    ...
    .forEach((c) => {
      ...
      const sub =
        c.type === 'private' && c.peer?.status_text
          ...
          : '';
      li.innerHTML = `
        <img class="avatar" alt="" src="${avatar ? window.location.origin + avatar : ''}" />
        <div class="meta">
...
function buildMessageElement(m) {
  ...
  if (m.msg_type === 'image' && m.file_path) {
    bodyHtml += `<br><img class="attach" src="${window.location.origin}${m.file_path}" alt="" loading="lazy" />`;
  } else if (m.file_path && m.file_mime?.startsWith('video/')) {
    bodyHtml += `<br><video class="attach" src="${window.location.origin}${m.file_path}" controls playsinline></video>`;
  } else if (m.file_path && m.file_mime?.startsWith('audio/')) {
    bodyHtml += `<br><audio class="attach" src="${window.location.origin}${m.file_path}" controls></audio>`;
  } else if (m.msg_type === 'file' && m.file_path && !m.file_mime?.startsWith('audio/')) {
    bodyHtml += `<br><a class="file-link" href="${window.location.origin}${m.file_path}" target="_blank" rel="noopener">${escapeHtml(
      m.file_name || 'File'
    )}</a>`;
  }
...
function appendMessage(chatId, m) {
  ...
  if (document.hidden && settings.notify_desktop && window.tmessing?.showNotification) {
    window.tmessing.showNotification('TMessing', `${m.username}: ${m.body || m.file_name || ''}`);
  }
...
$('#btn-profile')?.addEventListener('click', async () => {
  ...
  if (img) {
    img.src = u.avatar_url ? window.location.origin + u.avatar_url : '';
    img.alt = u.username;
  }
});
...
$('#profile-avatar-input')?.addEventListener('change', async (e) => {
  ...
  try {
    ...
    const img = $('#profile-avatar');
    if (img && res.avatar_url) img.src = window.location.origin + res.avatar_url;
    await refreshMe();
  } catch (err) {
    alert(err.data?.error || err.message);
  }
